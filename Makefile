...
build: src
	...
	cd ${BUILD} && ocamlfind ocamlopt -o xml2saty -linkpkg -package xml-light range.ml types.ml parse.mli parse.ml lex.ml error.mli error.ml optionState.mli optionState.ml satysfiSyntax.mli satysfiSyntax.ml configState.mli configState.ml configApply.mli configApply.ml main.ml
	cp ${BUILD}/${TARGET} ./
